...
function App() {
  ...
  const results = useMemo(
    ...
    [travelers, rates, status, manualMode, manualRates, euroForEurope, roundMan]
  );
  ...
  const saveTrip = async (asNew = false) => {
    ...
    if (dbClient) {
      if (overwrite) {
        const { error } = await dbClient.from(SB_TABLE).update(payload).eq("id", currentId);
        if (error) {
        ...
        const { data, error } = await dbClient.from(SB_TABLE).insert(payload).select();
        if (error) {
      ...
      if (overwrite) {
        const list = saved.map(
          (x) => x.id === currentId ? { ...x, tripName: name, count: payload.count, total: payload.total, travelers: payload.travelers, savedAt: (/* @__PURE__ */ new Date()).toISOString() } : x
        );
        setSaved(list);
        persistLocal(list);
        setNotice(`'${name}'\uC5D0 \uB36E\uC5B4\uC37C\uC2B5\uB2C8\uB2E4.`);
